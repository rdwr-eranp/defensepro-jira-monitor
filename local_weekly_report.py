...
def get_sprint_list(jira, n_closed=4):
    ...
    active sprint. Each entry is a dict: {name, startDate, endDate, state}.
...
def get_ci_sprint_history(conn, version, sprint_list, prev_version=None):
    """
    For each sprint compute CI metrics from PostgreSQL:
      coverage, pass_ratio, tests_executed, passed, failed.
    Sprints with no DB data are skipped. Returns [] when conn is None.
    """
    if conn is None:
        return []
    ...
    for sprint in sprint_list:
        ...
        try:
            ...
            pass_ratio = passed / max(total_exec, 1) * 100
            history.append({
...
def get_sub_exec_sprint_history(jira, version, sprint_list):
    """
    For each sprint query Jira for sub test execution completion snapshot at
    sprint end date. Returns a list of dicts per sprint.
    """
    history = []
    base_jql = (
    ...
    for sprint in sprint_list:
        ...
        if not s_end:
            continue
        try:
            total_res = jira.search_issues(
                base_jql + f' AND created <= "{s_end}"',
                maxResults=False, fields='status', json_result=True
            )
            total = total_res.get('total', 0)
            if total == 0:
                continue

            completed_res = jira.search_issues(
                base_jql + f' AND status in (Done, Accepted, Complete) AND resolved <= "{s_end}"',
                maxResults=False, fields='status', json_result=True
            )
            completed = completed_res.get('total', 0)

            in_progress_res = jira.search_issues(
                base_jql + f' AND status = "In Progress" AND created <= "{s_end}"',
                maxResults=False, fields='status', json_result=True
            )
            in_progress = in_progress_res.get('total', 0)
...
"""
        ...
        if ci_sprint_history:
            ...
            if len(ci_sprint_history) >= 2:
                ...
                if last_closed and last_closed != first:
                    context += (
                        f"  Coverage change over tracked sprints: {last_closed['coverage'] - first['coverage']:+.1f}%\n"
                        f"  Pass ratio change over tracked sprints: {last_closed['pass_ratio'] - first['pass_ratio']:+.1f}%\n"
                    )
        ...
        if historical_trends and historical_trends.get('dates'):
            ...
            if len(total_counts) >= 2:
                ...
                context += f"  Week-over-week change in total open bugs: {delta:+d}\n"
        ...
        if sub_exec_stats:
            ...
            not_start_se = sub_exec_stats.get('not_started', 0)
        ...
        if sub_exec_sprint_history:
            ...
            if len(sub_exec_sprint_history) >= 2:
                ...
                if last_closed_se and last_closed_se['completion_pct'] != first_pct:
                    context += f"  Completion rate change over tracked sprints: {last_closed_se['completion_pct'] - first_pct:+.1f}%\n"
...
def get_coverage_progress(conn, version, ci_run_start):
    ...
    Returns a dict with daily breakdown and completion estimation.
    """
    ...
    try:
        # Get baseline (available tests per platform_type/mode from previous version)
        baseline_query = f"""
            SELECT 
        ...
        # Get cumulative unique tests per day (per platform_type/mode for proper coverage)
        cumulative_query = f"""
        ...
        print(f"   \u26a0\ufe0f Coverage progress calculation failed: {e}")
        return None


def generate_coverage_progress_html(progress_data):
    """Generate HTML section for coverage progress and estimation."""
    if not progress_data:
        return ''

    days = progress_data['days']
    if not days:
        return ''
    ...
    # Estimation text
    if progress_data['days_to_complete']:
        est_html = (
            f'<div style="background: #e3f2fd; border-left: 5px solid #1565c0; padding: 15px; margin: 15px 0; border-radius: 4px;">'
            f'<strong>\U0001f4c8 Completion Estimate:</strong> At the current avg rate of '
...
def main():
    ...
    # Get bug data - all active bugs across the entire DP project (not version-scoped)
    ...
    if ci_run_start and conn:
        ...
        if coverage_progress:
            print(f"✓ Coverage progress: {coverage_progress['current_coverage']:.1f}% "
                  ...
                  f"est. completion: {coverage_progress['estimated_date'] or 'N/A'})\n")
        else:
    ...
    # Fetch bugs that were closed/accepted during this sprint period (separate query)
    ...
    coverage_progress_html = generate_coverage_progress_html(coverage_progress)
